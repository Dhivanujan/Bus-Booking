    ...
    <>
      <nav
        className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
          scrolled ? 'nav-glass shadow-[0_10px_30px_rgba(0,0,0,0.35)]' : 'bg-transparent'
        }`}
      >
        <div className="container">
          <div className="flex items-center justify-between h-18">
                ...
                return (
                  <button
                    ...
                    className={`relative text-[0.85rem] uppercase tracking-[0.18em] transition-colors ${
                      isActive ? 'text-white' : 'text-[#8FA0B8] hover:text-white'
                    }`}
                  >
                    {item.label}
                    <span
            ...
            <div className="flex items-center gap-3">
              <div className="hidden xl:flex items-center gap-2 text-xs uppercase tracking-[0.2em] text-[#8FA0B8]">
      ...
      >
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.2em] text-[#7E8CA5]">Navigation</p>
              <h2 className="mt-1 text-xl font-semibold text-white">Explore LuxeBus</h2>
            </div>
            <button
            ...
            {navItems.map((item) => (
              <button
                ...
                className={`flex items-center justify-between rounded-[20px] border border-white/10 bg-white/5 px-5 py-4 text-left text-sm uppercase tracking-[0.2em] transition-colors ${
                  active === item.id ? 'text-white' : 'text-[#8FA0B8] hover:text-white'
                }`}
              >
                {item.label}
                <ArrowRight size={16} />
          ...
          <button onClick={() => scrollTo('search')} className="btn-primary btn-primary--sm w-full mt-6">
            Book now <ArrowRight size={16} />
          </button>
        </div>
      </div>
